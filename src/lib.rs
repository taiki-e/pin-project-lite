...
macro_rules! __pin_project_internal {
    ...
    (@callback_default;
        ...
        $crate::__pin_project_internal! {@$proj_ty=>$callback; [$target] $($args)*}
    };
    (@callback_default;
    ...
    // catch a conditional projection
    (@triggered=>project_and_construct;
        [$proj_ident:ident]
    ...
    // structs do not need push down accumulation
    ...
    (@project_and_construct;
        ...
        $crate::__pin_project_internal! {@project_and_construct;
            ...
            [$projection {
                $($pout)*
                $($field_vis $field: $crate::__pin_project_internal! {@$projection; $(#[$pin])? $field_ty}),+
            }]
            [raw {}]
        }
    };
    ...
    (@construct;
        ...
        $(#[$attrs])*
        $vis $struct_ty_ident $ident $($def_generics)*
        $(where
            $($where_clause)*)?
        {
            $($body_data)*
    ...
    (@construct;
        ...
        #[allow(clippy::type_repetition_in_bounds)] // https://github.com/rust-lang/rust-clippy/issues/4326
        $vis $struct_ty_ident $proj_ty_ident <'__pin, $($impl_generics)*>
        where
    ...
    (@expand;
        ...
        $crate::__pin_project_internal! {@project_and_construct;
            [$vis $struct_ty_ident $ident $ident]
            [meta $([$($meta_data)*])*]
            [reconstruction {}]
            [raw {$($body_data)+}]
        }
    ...
    (@expand=>constant;
        ...
        $crate::__pin_project_internal! { @expand=>constant;
            [struct
                [$vis $ident $proj_vis]
                [mut_ident $($proj_mut_ident)?; ref_ident $($proj_ref_ident)?; replace_ident $($proj_replace_ident)?]]
            [default_idents Projection ProjectionRef ProjectionReplace]
            [meta $([$($meta_data)*])*]
            [pinned_drop $(impl $($pinned_drop)*)?]
            [body $($body_data)+]
        }
    ...
    (@expand=>constant;
        [struct
            [$vis:vis $ident:ident $proj_vis:vis]
            [mut_ident $($proj_mut_ident:ident)?; ref_ident $($proj_ref_ident:ident)?; replace_ident $($proj_replace_ident:ident)?]]
        [default_idents $default_proj_mut:ident $default_proj_ref:ident $default_proj_repalce:ident]
        [meta $([$($meta_data:tt)*])*]
        [pinned_drop $(impl $($pinned_drop:tt)*)?]
        [body $($body_data:tt)+]
    ) => {
        ...
        const _: () = {
            ...
            $crate::__pin_project_internal! { @oldstyle=>constant;
                [struct
                    [$vis $ident $proj_vis]
                    [mut_ident $($proj_mut_ident)?; ref_ident $($proj_ref_ident)?; replace_ident $($proj_replace_ident)?]]
                [default_idents $default_proj_mut $default_proj_ref $default_proj_repalce]
                [meta $([$($meta_data)*])*]
                [pinned_drop $(impl $($pinned_drop)*)?]
                [body $($body_data)+]
            }
        ...
        [body
                ...
                $($where_clause)*)?
            {
                $crate::__pin_project_internal! { @callback_default;
                    [$($proj_mut_ident)? default $default_proj_mut]
                ...
                $crate::__pin_project_internal! { @callback_default;
                    [$($proj_ref_ident)? default $default_proj_ref]
                    [cb struct make_proj_method]
                    [args
                        [$proj_vis]
                        [project_ref get_ref]
    ...
    // =============================================================================================
    // removes pin_project attrs from the type
    (@make_clean_field;
        $(#[pin])? $field_ty:ty
    ) => {
        $field_ty
