...
macro_rules! __pin_project_internal {
    ...
    (@enum_newtype=>internal;
        ...
        $crate::__pin_project_internal! { @enum_newtype=>make_proj_ty;
            [$proj_vis]
            [$($proj_mut_ident)?]
            [make_proj_field_mut]
            [$ident]
        ...
        $crate::__pin_project_internal! { @enum_newtype=>make_proj_replace_ty;
            [$proj_vis]
            [$($proj_replace_ident)?]
            [make_proj_field_replace]
            [$ident]
        ...
        const _: () = {
            ...
            $(where
                $($where_clause)*)?
            {
                // WARNING, these next three are replacing field with the typename, i probably can *remove* the $field_ty
                $crate::__pin_project_internal! { @enum_newtype=>make_proj_method;
                    [$proj_vis]
                    [$($proj_mut_ident)?]
                    [project get_unchecked_mut mut]
                    [$($ty_generics)*]
                ...
                $crate::__pin_project_internal! { @enum_newtype=>make_proj_replace_method;
                    [$proj_vis]
                    [$($proj_replace_ident)?]
                    [$($ty_generics)*]
                    {
            ...
            // TODO: I might need to de-array this because one, I have not touched it and really there is only one field
            $crate::__pin_project_internal! { @make_unpin_impl;
                [$vis $ident]
                [$($impl_generics)*] [$($ty_generics)*] [$(where $($where_clause)*)?]
                $(
    ...
    (@enum_newtype=>make_proj_ty;
            ...
            $(, $($where_clause)*)?
        {
            $(
                ...
                $variant $((
                    $crate::__pin_project_internal!(@$make_proj_field;
                        $(#[$pin])? $field_ty
                    )
                ))?
            ),+
    ...
    (@enum_newtype=>make_proj_replace_ty;
        [$proj_vis:vis]
        [$proj_ty_ident:ident]
        [$make_proj_field:ident]
        [$ident:ident]
    ...
    (@enum_newtype=>make_proj_replace_method;
        [$proj_vis:vis]
        [$proj_ty_ident:ident]
        [$($ty_generics:tt)*]
        {
                ...
                match &mut *__self_ptr {
                    $(
                        Self::$variant $((
                            ...
                            // TODO: if it fails, i wonder if the newline needs to be respected
                            $crate::__pin_project_internal!{@make_proj_replace_block;
                                [$proj_ty_ident :: $variant]
                                $((
                                    $(#[$pin])? $variant_value
    ...
    (
        ...
        $crate::__pin_project_internal! { @enum_newtype=>internal;
            [$($proj_mut_ident)?]
            [$($proj_ref_ident)?]
            [$($proj_replace_ident)?]
            [pub(crate)]
